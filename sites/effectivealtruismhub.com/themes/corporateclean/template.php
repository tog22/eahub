...
function corporateclean_preprocess_node($vars) {
	// to see what's available in $vars, run: 
	// watchdog('corporateclean_preprocess_node',print_r($vars,TRUE));
	
	/* Tried moving to html.tpl.php, but gives odd $node non-object messages: */
	// If we're on the front page or map page, include the necessary CSS for this
	if ($vars['nid'] == 2 || $vars['nid'] == 13 || $vars['nid'] == 23 || $vars['nid'] == 91 || $vars['nid'] == 170) {
	// **NB** in preprocess_page, could also do if (drupal_get_path_alias("node/{$vars['#node']->nid}") == 'foo') 
		// debugging: drupal_set_message(t("Internal testing message: on node 23; code v4"), 'status');
		drupal_add_css("http://cdn.leafletjs.com/leaflet-0.7.3/leaflet.css",'external');
		drupal_add_css("http://cdnjs.cloudflare.com/ajax/libs/leaflet.markercluster/0.4.0/MarkerCluster.Default.css",'external');
		//drupal_add_css("http://cdnjs.cloudflare.com/ajax/libs/leaflet.markercluster/0.4.0/MarkerCluster.css",'external');
		drupal_add_css("http://maxcdn.bootstrapcdn.com/font-awesome/4.2.0/css/font-awesome.min.css",'external');
		drupal_add_css('http://cdnjs.cloudflare.com/ajax/libs/Leaflet.awesome-markers/2.0.0/leaflet.awesome-markers.css','external');
